...
BREW_PACKAGE="hduhelp/tap/multica"
...
print_remote_server_token_hint() {
  ...
  printf "     1. On your local computer, open ${CYAN}https://multica.hduhelp.com/settings?tab=tokens${RESET}\n"
...
install_cli_brew() {
  ...
  local brew_log
  brew_log=$(mktemp)
  if ! brew tap hduhelp/tap >"$brew_log" 2>&1; then
...
install_cli_binary() {
  ...
  local url="https://github.com/hduhelp/multica/releases/download/${latest}/multica-cli-${version}-${OS}-${ARCH}.tar.gz"
  local tmp_dir
  tmp_dir=$(mktemp -d)
...
run_default() {
  ...
  printf "\n"
  printf "     ${CYAN}multica setup${RESET}                # Connect to Multica Cloud (multica.hduhelp.com)\n"
