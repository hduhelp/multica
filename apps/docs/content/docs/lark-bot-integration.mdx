...
The agent replies onto your message, opening a Feishu topic for that exchange. Follow-ups inside the topic continue the same agent session, so a back-and-forth keeps its context instead of starting over. Once a topic is running the Bot stops re-quoting the parent message.

Reply format follows the result: a short answer comes back as a plain message, a longer one as a formatted card, and a failed run as a separate error card.
