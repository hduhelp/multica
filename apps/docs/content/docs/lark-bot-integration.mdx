...
| **Threaded reply** | The Bot replies onto your message, opening (or continuing) a Lark topic (话题) so each conversation stays in its own thread. A short answer comes back as a plain message, a richer reply as a formatted card, and a failed run as a distinct error card. |
...
- **Ask the agent anything** — DM the Bot or @-mention it in a group. The conversation is a normal agent chat session; the agent replies threaded onto your message.
...
- **Stay in context** — each reply threads onto your message, opening or continuing a Lark topic (话题). Follow-ups inside that topic continue the same agent session, so a back-and-forth keeps its context instead of starting over.
