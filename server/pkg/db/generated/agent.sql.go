...
WHERE id = $1
RETURNING id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, thinking_level, composio_toolkit_allowlist, permission_mode, kind, system_key, disabled_runtime_skills, service_tier, starter_prompts, fixed_repo_enabled, fixed_repo_paths, fixed_repo_vcs_type, fixed_repo_cleanup_script, fixed_repo_worktree
`
...
type CreateAgentParams struct {
	...
	FixedRepoWorktree        interface{} `json:"fixed_repo_worktree"`
}
...
SELECT id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, thinking_level, composio_toolkit_allowlist, permission_mode, kind, system_key, disabled_runtime_skills, service_tier, starter_prompts, fixed_repo_enabled, fixed_repo_paths, fixed_repo_vcs_type, fixed_repo_cleanup_script, fixed_repo_worktree FROM agent
WHERE id = $1
`
...
UPDATE agent SET
    ...
    fixed_repo_enabled = COALESCE($21, fixed_repo_enabled),
    fixed_repo_paths = COALESCE($22, fixed_repo_paths),
    fixed_repo_vcs_type = COALESCE($23, fixed_repo_vcs_type),
    fixed_repo_cleanup_script = COALESCE($24, fixed_repo_cleanup_script),
    fixed_repo_worktree = COALESCE($25, fixed_repo_worktree),
    updated_at = now()
WHERE id = $1
RETURNING id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, thinking_level, composio_toolkit_allowlist, permission_mode, kind, system_key, disabled_runtime_skills, service_tier, starter_prompts, fixed_repo_enabled, fixed_repo_paths, fixed_repo_vcs_type, fixed_repo_cleanup_script, fixed_repo_worktree
`
...
type UpdateAgentParams struct {
	...
	FixedRepoCleanupScript   pgtype.Text `json:"fixed_repo_cleanup_script"`
	FixedRepoWorktree        pgtype.Bool `json:"fixed_repo_worktree"`
}
