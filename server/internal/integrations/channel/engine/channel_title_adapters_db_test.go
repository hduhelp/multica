...
func TestChannelStartTitleCommandMappingDB(t *testing.T) {
	...
	for _, platform := range []string{"feishu", "telegram", "dingtalk", "slack", "wecom"} {
		t.Run(platform, func(t *testing.T) {
			...
			switch platform {
			case "feishu":
				// The trailing api/creds pair is this fork's agent-sender lookup; nil
				// keeps the identity resolver on its person-only path, which is
				// all a session-binder test needs.
				binder = lark.NewFeishuResolverSet(nil, session, nil, nil, nil, nil, nil, nil).Session
			case "telegram":
