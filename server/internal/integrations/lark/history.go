...
func (h *History) ChannelOverview(ctx context.Context, chatSessionID pgtype.UUID, opts channel.HistoryOptions) (channel.HistoryPage, error) {
	...
	// Upstream's client fetches one page and neither takes nor returns a page
	// token, so opts.Before is not honoured and NextCursor stays empty: this
	// reads the most recent Limit messages, not an arbitrary window.
	msgs, err := h.client.ListChatMessages(ctx, t.creds, ListMessagesParams{
	...
	page := h.normalizePage(ctx, t, msgs, true)
...
func (h *History) Thread(ctx context.Context, chatSessionID pgtype.UUID, threadID string, opts channel.HistoryOptions) (channel.HistoryPage, error) {
	...
	// The chat container is always the session's pinned chat; ThreadID scopes
	// the read to one topic when there is one. Same single-page limitation as
	// ChannelOverview: no cursor in, no cursor out.
	params := ListMessagesParams{ChatID: ChatID(t.chatID), PageSize: limit}
	if ts != "" {
		params.ThreadID = ts
	}
	msgs, err := h.client.ListChatMessages(ctx, t.creds, params)
	if err != nil {
		return channel.HistoryPage{}, readError("read feishu thread", err)
	}
	page := h.normalizePage(ctx, t, msgs, false)
	page.ChannelType = string(channel.TypeFeishu)
	page.ThreadID = ts
	return page, nil
}
