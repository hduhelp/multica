...
// TestPatcherOpensTopicInOrdinaryGroup pins this fork's divergence from
// upstream #8234. Upstream replies natively here and leaves reply_in_thread
// false; this fork sends it true so the reply OPENS a 话题, because
// larkSessionRouting keys a group conversation on the thread root and the root
// only becomes a thread by this send.
func TestPatcherOpensTopicInOrdinaryGroup(t *testing.T) {
	p, q, api := newTestPatcher(t)
	...
	got := api.textSent[0].ReplyTarget
	if got.MessageID != "om_trigger" || !got.InThread {
		t.Errorf("group reply must open a topic off the trigger {om_trigger, InThread:true}; got %+v", got)
	}
}
...
// at all, or a session whose trigger was never recorded. ReplyTarget stays
// empty and the send goes to the chat by chat_id.
