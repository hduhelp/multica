...
func TestFeishuSessionBinder_StartSessionMapping(t *testing.T) {
	...
	got := f.startIn
	// This fork keys a group conversation on the thread ROOT message, not on
	// the topic id upstream uses. The bot opens the topic itself by replying in
	// thread, so at the first @-mention no topic exists yet and Source.ThreadID
	// is empty; keying on the topic id would file that opening turn under a
