...
// historyFakeClient implements APIClient: it records ListChatMessages calls and
// returns a canned page, and resolves names from a fixed map.
type historyFakeClient struct {
	APIClient // embedded stub for the unused methods
	calls     []ListMessagesParams
	result    []LarkMessage
	listErr   error
...
func TestHistoryChannelOverview(t *testing.T) {
	...
	// Upstream's client fetches one page and returns no token, so a read never
	// advertises a cursor.
	if page.NextCursor != "" {
		t.Errorf("NextCursor = %q, want empty (client is single-page)", page.NextCursor)
	}
	if len(client.calls) != 1 || client.calls[0].ChatID != "oc_chat" ||
		client.calls[0].ThreadID != "" || client.calls[0].PageSize != 5 {
...
func TestHistoryThreadCurrentTopic(t *testing.T) {
	client := newHistoryFakeClient()
	client.result = []LarkMessage{historyMsg("m1", "ou_a", "user", "1", "th1", "hi")}
	...
	if len(client.calls) != 1 || client.calls[0].ThreadID != "th1" {
...
func TestHistoryThreadFallsBackToChat(t *testing.T) {
	client := newHistoryFakeClient()
	client.result = []LarkMessage{historyMsg("m1", "ou_a", "user", "1", "", "hi")}
	...
	if len(client.calls) != 1 || client.calls[0].ThreadID != "" || client.calls[0].ChatID != "oc_chat" {
...
func TestHistoryThreadByID(t *testing.T) {
	client := newHistoryFakeClient()
	client.result = []LarkMessage{historyMsg("m1", "ou_a", "user", "1", "tX", "hi")}
	h := newTestHistory(historyFakeQueries{
		binding: ChatSessionBinding{ChannelChatID: "oc_chat", LastThreadID: pgtype.Text{String: "th1", Valid: true}},
		inst:    activeInstall(),
	}, client)
	...
	if len(client.calls) != 1 || client.calls[0].ThreadID != "tX" {
