...
// Renamed from upstream's ...RepliesNativelyInOrdinaryGroup: this fork opens a
// 话题 here instead of replying flat, and the replier must match the patcher.
func TestLarkOutcomeReplierOpensTopicInOrdinaryGroup(t *testing.T) {
	t.Parallel()
	log := slog.New(slog.NewTextHandler(io.Discard, nil))
	stub := &stubAPIClientWithRecorder{configured: true}
	rep := NewLarkOutcomeReplier(OutcomeReplierConfig{
	...
	if got.MessageID != "om_trigger" || !got.InThread {
		t.Errorf("ordinary group confirmation must open a topic off the trigger; got %+v", got)
	}
}
