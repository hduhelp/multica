...
// ListChatBots lists the bots that are members of a chat, via
// GET /open-apis/im/v1/chats/{chat_id}/members/bots.
//
// Lark returns bot_id in the CALLING app's namespace plus bot_name, and
// nothing else — there is no app_id and no cross-app identifier anywhere in
// this API surface. Callers therefore join on the name, and must read both
// sides of that join from this same roster so the two names come from one
...
func (c *httpAPIClient) ListChatBots(ctx context.Context, creds InstallationCredentials, chatID ChatID) ([]ChatBotMember, error) {
	...
	// NOT im/v1/chats/{id}/members — that endpoint returns human members and
	// silently ignores a member_type=bot filter, which is how the first
	// attempt at this shipped a lookup that could never match anything.
	path := "/open-apis/im/v1/chats/" + url.PathEscape(string(chatID)) + "/members/bots"
	...
	for _, it := range resp.Data.Items {
		...
		out = append(out, ChatBotMember{OpenID: it.BotID, Name: it.BotName})
	}
	return out, nil
}
