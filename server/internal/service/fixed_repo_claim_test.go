...
func TestFixedRepoClaimAssignsDistinctPathsAndBlocks(t *testing.T) {
	...
	if _, err := svc.CompleteTask(ctx, first.ID, []byte(`{}`), "", "/data/repos/alpha", "", false, "", ""); err != nil {
...
func TestFixedRepoLockReleasedOnFailAndCancel(t *testing.T) {
	...
	if _, err := svc.FailTask(ctx, first.ID, "boom", "", "/data/repos/alpha", "", "agent_error", false, "", ""); err != nil {
