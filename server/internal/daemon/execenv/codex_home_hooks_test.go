...
func TestPrepareCodexHomeFailsLoudWhenSharedHooksExposureFails(t *testing.T) {
	...
	// A read-only per-task codex-home fails loud either at hooks exposure or,
	// now that #5484 provisions auth into the same dir first, at auth-file
	// provisioning — both are the intended fail-closed outcome for an unwritable
	// codex-home, so accept either.
	if !strings.Contains(err.Error(), "hooks.json") &&
		!strings.Contains(err.Error(), "optional file") &&
		!strings.Contains(err.Error(), "provision codex auth") {
		t.Fatalf("error should reference the hooks exposure or auth provisioning failure, got: %v", err)
	}
}
