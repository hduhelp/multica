...
// TestEnsureIssueWorktree_ReclaimsRegisteredBrokenWorktree verifies the
// restart-resilience self-heal: a worktree git registered but whose checkout
// became unusable (here: its .git link removed, simulating a crashed
// `git worktree add`) is reclaimed and recreated on the next ensure, instead of
// wedging the issue forever. Distinct from an unregistered foreign directory,
// which stays fail-closed (see TestEnsureIssueWorktree_RejectsUnexpectedCanonicalDirectory).
func TestEnsureIssueWorktree_ReclaimsRegisteredBrokenWorktree(t *testing.T) {
	repo := initLocalRepo(t)
	root := t.TempDir()
	d := &Daemon{cfg: Config{WorkspacesRoot: root, DaemonID: "d-test"}, logger: slog.Default(), repoGitLocks: NewLocalPathLocker()}

	// First ensure creates + registers the worktree.
	wtPath, _, err := d.ensureIssueWorktree(context.Background(), repo, "issue-1", "")
	if err != nil {
		t.Fatalf("first ensure: %v", err)
	}
	// It must remain registered in the base repo.
	if reg, regErr := worktreeIsRegistered(context.Background(), repo, wtPath); regErr != nil || !reg {
		t.Fatalf("expected worktree registered: reg=%v err=%v", reg, regErr)
	}
	// Break the checkout: remove the .git link so validateIssueWorktree fails,
	// while the base repo still lists the worktree as registered.
	if err := os.Remove(filepath.Join(wtPath, ".git")); err != nil {
		t.Fatalf("break worktree: %v", err)
	}

	// Second ensure must self-heal: reclaim the broken registration and recreate.
	wt2, _, err := d.ensureIssueWorktree(context.Background(), repo, "issue-1", "")
	if err != nil {
		t.Fatalf("second ensure should self-heal, got: %v", err)
	}
	if wt2 != wtPath {
		t.Errorf("reclaim changed path: %q → %q", wtPath, wt2)
	}
	if !isGitWorktreeDir(wt2) {
		t.Errorf("expected a valid linked worktree after reclaim at %q", wt2)
	}
}
