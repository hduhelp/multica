...
// fixed-repo task carrying an issue resolves to the worktree execution mode, so
// the daemon's existing worktree machinery isolates it instead of serializing it
// on the in_place path mutex.
func TestFixedRepoAssignmentForTask_WorktreeWithIssue(t *testing.T) {
	...
	if !got.UsesWorktree() {
		t.Errorf("UsesWorktree() = false, want true (execution_mode = %q)", got.Ref.ExecutionMode)
	}
}
...
func TestFixedRepoAssignmentForTask_WorktreeWithoutIssue(t *testing.T) {
	...
	if got.UsesWorktree() {
		t.Error("UsesWorktree() = true for a task with no issue; want in_place")
	}
}
...
func TestFixedRepoAssignmentForTask_NonWorktreeDefaultsInPlace(t *testing.T) {
	...
	if got.UsesWorktree() {
		t.Errorf("UsesWorktree() = true for a non-worktree fixed repo task (execution_mode = %q)", got.Ref.ExecutionMode)
	}
}
