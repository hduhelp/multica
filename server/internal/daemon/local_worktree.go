...
func (d *Daemon) ensureIssueWorktree(ctx context.Context, localRepo, issueID, baseRef string) (wtPath, branch string, err error) {
	...
	if validateErr != nil {
		// Self-heal (restart resilience): a crashed `git worktree add` can leave
		// a partial directory at the canonical path that is NOT a usable linked
		// worktree. Left as-is it would wedge every future task for this issue
		// forever — the directory persists on disk, so even a daemon restart
		// wouldn't clear it. But we must NOT blindly delete whatever squats the
		// path: foreign data (a bug, an operator, an attacker) could be there.
		// The safe discriminator is the base repo's own worktree registry: only
		// a directory git ITSELF registered as a worktree of this repo is a
		// daemon-created leftover we may reclaim. Anything git never registered
		// is left untouched and surfaced (fail closed).
		if registered, regErr := worktreeIsRegistered(ctx, localRepo, wtPath); regErr != nil || !registered {
			return "", "", validateErr
		}
		d.logger.Warn("local_directory worktree: reclaiming registered-but-broken worktree",
			"path", wtPath, "reason", validateErr)
		// `git worktree remove --force` unregisters and deletes the broken
		// worktree; prune sweeps any residual registration. Fall through to
		// recreate afterward.
		if out, rmErr := exec.CommandContext(ctx, "git", "-C", localRepo, "worktree", "remove", "--force", wtPath).CombinedOutput(); rmErr != nil {
			d.logger.Warn("local_directory worktree: git worktree remove during reclaim failed; falling back to prune",
				"error", rmErr, "output", strings.TrimSpace(string(out)))
			_ = os.RemoveAll(wtPath)
		}
		if out, pruneErr := exec.CommandContext(ctx, "git", "-C", localRepo, "worktree", "prune").CombinedOutput(); pruneErr != nil {
			d.logger.Warn("local_directory worktree: prune after reclaim failed",
				"error", pruneErr, "output", strings.TrimSpace(string(out)))
		}
	} else if exists {
...
// worktreeIsRegistered reports whether the base repo's worktree registry lists
// wtPath — i.e. git itself created a worktree there (possibly since broken).
// This is the safe discriminator for the self-heal reclaim path: a registered
// entry is daemon/git-created and may be removed, whereas an unregistered
// directory is foreign and must be left untouched. Compares resolved absolute
// paths so symlinked temp roots (macOS /var → /private/var) still match.
func worktreeIsRegistered(ctx context.Context, localRepo, wtPath string) (bool, error) {
	out, err := exec.CommandContext(ctx, "git", "-C", localRepo, "worktree", "list", "--porcelain").CombinedOutput()
	if err != nil {
		return false, fmt.Errorf("git worktree list: %s: %w", strings.TrimSpace(string(out)), err)
	}
	target := resolvePathForCompare(wtPath)
	for _, line := range strings.Split(string(out), "\n") {
		if !strings.HasPrefix(line, "worktree ") {
			continue
		}
		listed := strings.TrimSpace(strings.TrimPrefix(line, "worktree "))
		if resolvePathForCompare(listed) == target {
			return true, nil
		}
	}
	return false, nil
...
// resolvePathForCompare returns a canonical form for path comparison: symlinks
// resolved when possible, otherwise the cleaned absolute path.
