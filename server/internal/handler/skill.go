...
func discoverGitHubDirectorySkills(ctx context.Context, httpClient *http.Client, rawURL string) ([]SkillSearchCandidateResponse, error) {
	spec, err := parseGitHubURL(rawURL)
	if err != nil {
		return nil, err
	}
	...
	// Reuse the tree-first fetch (one recursive git-trees request) that the
	// single-skill importer uses (MUL-5136), instead of a second hand-rolled
	// tree call.
	entries, _, err := fetchGitHubTree(ctx, httpClient, spec.owner, spec.repo, spec.ref)
	if err != nil {
		return nil, err
	}
	...
	subs := immediateSubSkillDirs(extractSkillMdPaths(entries), prefix)
...
func (h *Handler) ImportSkill(w http.ResponseWriter, r *http.Request) {
	...
	case sourceGitHub:
		...
		if err != nil {
			...
			if candidates, derr := discoverGitHubDirectorySkills(ctx, httpClient, normalized); derr == nil && len(candidates) > 0 {
