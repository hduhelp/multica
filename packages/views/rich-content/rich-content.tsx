...
function RichLink({ href, children }: { href?: string; children?: ReactNode }) {
  ...
  if (isMentionHref(href)) {
    ...
    // Member / agent / squad / @all mentions render as avatar chips (#5199).
    // Readonly chips are non-focusable; the chip's aria-label conveys identity
    // and mouse hover still opens the MentionHoverCard.
    if (match?.[1] && match[2] && isActorMentionType(match[1])) {
      const rawLabel = childrenToLabel(children) ?? match[2];
      // The mention markdown serializes the label WITH a leading "@"; the chip
      // adds its own "@" prefix, so strip one leading "@" to avoid "@@".
